...
% It has been defined as a ONE-staged optimization.
...
y0BC1 = linspace(ymin+dy,ymax-dy,ynum);
y0BC2 = linspace(ymax+dy,ymin-dy,ynum);
...
% Set the value of x for the x-Boundary Conditions
x0BC3 = linspace(xmin+dx,xmax-dx,xnum);
x0BC4 = linspace(xmax+dx,xmin-dx,xnum);
% Set the value of the function as zero at the BC (Dirichlet BCs)
u0BC3 = zeros(1,xnum);
u0BC4 =  ones(1,xnum);
v0BC3 = zeros(1,xnum);
...
Xc = xmin+dx:dx:xmax-dx;      % (xmin+dx):dx:(xmax-dx)
Yc = ymin+dy:dy:ymax-dy;
...
options = optimoptions('fmincon', ... % Using the multivariate restricted optimization function fmincon
    ...
    'OptimalityTolerance',1e-10, ... % By default considering a tolerance of 0.00001
    'SpecifyObjectiveGradient',true,... % User-defined gradient for the algorithm
    'PlotFcn',@optimplotfval,... % Plotting the first-order optimality across iterations
...
%     pp.("Trial_"+i) = parameters;
%     % e. Implement the Model Training for the Hyperbolic Arctangent AF
%     [parameters_fin,output,fval] = PINN_HE_Dirichlet(inputs,parameters,options);
%     fv(i) = fval;
%     disp(i);
% end
...
function [gradients,loss] = modelGradients(parameters,dlXBC,dlYBC,dlUBC,dlVBC,dlXP,dlYP,dlPP,dlX,dlY)
    ...
    % Combine losses.
    loss = (lossF + lossG + lossH) + lossP + 10*lossU + lossV;
...
function [parameters,output,fval] = PINN_HE_Dirichlet(in,parameters,options)
...
[parametersV,fval,~,output,~,~,~] = fmincon(objFun,parametersV,[],[],[],[],[],[],[],options);
toc
