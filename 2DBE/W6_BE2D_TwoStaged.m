...
% This is the baseline code for the 2DBE but with a two-staged
% implementation. A particular solution to this problem is contained in the
% file "Solution_BE2D.mat".
...
%% Checkpoint for results
% Load the file "Solution_BE2D.mat" here.
...
axis square
k = 1
error_U_IC = mse(k*Uu-sin(pi*Xx).*cos(pi*Yy))
error_V_IC = mse(k*Vv-cos(pi*Xx).*sin(pi*Yy))
subplot(2,3,3)
surf(Xx,Yy,abs(k*Uu-sin(pi*Xx).*cos(pi*Yy)))
xlabel('x','Interpreter','latex')
ylabel('y','Interpreter','latex')
zlabel('u','Interpreter','latex')
str = "Difference in Velocities (u)"; % with error = " + error_U_IC; 
...
shading interp
axis square
caxis([0,max(max(abs(k*Uu-sin(pi*Xx).*cos(pi*Yy))'))]);
subplot(2,3,6)
surf(Xx,Yy,abs(k*Vv-cos(pi*Xx).*sin(pi*Yy)))
xlabel('x','Interpreter','latex')
ylabel('y','Interpreter','latex')
zlabel('v','Interpreter','latex')
str = "Difference in Velocities (v)"; % with error = " + error_V_IC; 
...
shading interp
axis square
caxis([0,max(max(abs(k*Vv-cos(pi*Xx).*sin(pi*Yy))'))]);
...
%
function [loss,gradientsV] = objectiveFunction1(parametersU,dlX,dlY,dlT,dlXIC,dlYIC,dlTIC,dlUIC,dlVIC,dlXBCu,dlYBCu,dlTBCu,dlXBCv,dlYBCv,dlTBCv,dlUBC,dlVBC,parameterNames,parameterSizes)
    % Rate of Decay
    a = 0.9;
    ...
    [gradients,loss] = dlfeval(@modelGradients1,parametersu,dlX,dlY,dlT,dlXIC,dlYIC,dlTIC,dlUIC,dlVIC,dlXBCu,dlYBCu,dlTBCu,dlXBCv,dlYBCv,dlTBCv,dlUBC,dlVBC);
    % Return loss and gradients for fmincon.
    gradientsV = parameterStructToVector(gradients);
    gradientsV = a*extractdata(gradientsV);
    loss = extractdata(loss);
end
function [loss,gradientsV] = objectiveFunction2(parametersU,dlX,dlY,dlT,dlXIC,dlYIC,dlTIC,dlUIC,dlVIC,dlXBCu,dlYBCu,dlTBCu,dlXBCv,dlYBCv,dlTBCv,dlUBC,dlVBC,parameterNames,parameterSizes)
    % Rate of Decay
    a = 0.9;
    % Convert parameters to structure of dlarray objects.
    parametersU = dlarray(parametersU);
    parametersu = parameterVectorToStruct(parametersU,parameterNames,parameterSizes);
    % Evaluate model gradients and loss.
    [gradients,loss] = dlfeval(@modelGradients2,parametersu,dlX,dlY,dlT,dlXIC,dlYIC,dlTIC,dlUIC,dlVIC,dlXBCu,dlYBCu,dlTBCu,dlXBCv,dlYBCv,dlTBCv,dlUBC,dlVBC);
    % Return loss and gradients for fmincon.
    gradientsV = parameterStructToVector(gradients);
    gradientsV = a*extractdata(gradientsV);
    loss = extractdata(loss);
...
%
function [gradients,loss] = modelGradients1(parameters,dlX,dlY,dlT,dlXIC,dlYIC,dlTIC,dlUIC,dlVIC,dlXBCu,dlYBCu,dlTBCu,dlXBCv,dlYBCv,dlTBCv,dlUBC,dlVBC)
    ...
    % Combine losses.
    loss = (lossI + lossJ); % + 0.7*(lossB + lossW);
    % Calculate gradients with respect to the learnable parameters.
    gradients = dlgradient(loss,parameters);
end
function [gradients,loss] = modelGradients2(parameters,dlX,dlY,dlT,dlXIC,dlYIC,dlTIC,dlUIC,dlVIC,dlXBCu,dlYBCu,dlTBCu,dlXBCv,dlYBCv,dlTBCv,dlUBC,dlVBC)
    % Make predictions with the initial conditions.
    UU = model(parameters,dlX,dlY,dlT,1);
    VV = model(parameters,dlX,dlY,dlT,2);
    % Assume a value for the parameter Re
...
objFun = @(parameters) objectiveFunction1(parameters,dlX,dlY,dlT,...
...
objFun = @(parameters) objectiveFunction2(parameters,dlX,dlY,dlT,...
    dlXIC,dlYIC,dlTIC,dlUIC,dlVIC,dlXBCu,dlYBCu,dlTBCu,...
    dlXBCv,dlYBCv,dlTBCv,dlUBC,dlVBC,parameterNames,parameterSizes);
