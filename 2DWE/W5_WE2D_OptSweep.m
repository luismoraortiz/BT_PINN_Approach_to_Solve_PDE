...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   The Partial Differential Equation is the WAVE EQUATION
%   The Activation function is the ARCTANGENT
%   The Boundary Conditions are DIRICHLET
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% This code corresponds to the sweep of the 2D Wave Equation, solved 100
% times with the so called Optimal Architecture. Its data will be reachable
% through the .mat file: "Data_WE2D_OptSweep.mat".
...
%% Generating the average values in a new struct:
        ...
        for i = 1:100
            ...
            convdpercent = convdpercent + (errBCs(i)<0.02)&(errICs(i)<0.02); % convdpercent + convd.("NL_" + nL + "_NN_" + nN + "_TR_" + i);
            convds(i) = (errBCs(i)<0.01)&(errICs(i)<0.01);
        end
...
figure
hold on
scatter(errICs(convds==1),errBCs(convds==1),'b');
scatter(errICs(convds==0),errBCs(convds==0),'r');
hold off
...
figure
hold on
scatter(iters(convds==1),ttakens(convds==1),'b');
scatter(iters(convds==0),ttakens(convds==0),'r');
hold off
...
function [gradients,loss] = modelGradients(parameters,dlX,dlY,dlT,dlXIC,dlYIC,dlTIC,dlUIC,dlXBC,dlYBC,dlTBC,dlUBC,a)
    ...
    lossI = mse(dlUICPred, dlUIC) + mse(Un,0*dlUIC);
