...
%   This is the problem resolution in TWO-DIMENSIONS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Generating the interior-point matrices
...
%% This section obtains the coefficients for the Fourier expansion. 
% It is a costly section of the algorithm, for a more efficient
% implementation, execute one and store the data, loading them every time
% they are needed.
for i = 0:50
    for j = 0:50
        A = (2 - (i==0)).*(2 - (j==0));
...
for i = 1:numel(tTest)
    ...
    TTest = t*ones(size(XTest));
    alpha = 25;     k = 0.1;
    UAnal = solveHeatEquation(XTest,YTest,TTest,alpha,k);
    Uu = [];
    for s = 1:length(dlXTest)
        ii = ceil(s/width(YT)); % iii(s) = ii;
        jj = mod(s,width(YT));  % jjj(s) = jj;
...
for i = 1:3
    ...
    surf(Xx,Yy,Uv);
    title("Relative $L_{\infty}$ error at t = " + t(i),'Interpreter','latex');
    colorbar
    shading interp
end
