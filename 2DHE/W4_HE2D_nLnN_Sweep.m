...
%   The dimensionality of the problem is TWO-DIMENSIONS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% This code will be used to solve the three-dimensional wave equation on
% the context of a varying number of neurons and layers (nL, nN) in order
% to study the sensitivity of those two parameters on model performance in
% a particular problem. Accordingly, the dataset containing the data will
% be labelled "Data_HE2D_nN_nL_Sweep.mat"
...
convd = struct;
%%
for nL = 2:numLayersMax % The number of layers
    for nN = 1:numNeuronsMax % The number of neurons
        for i = 1:30 % The number of local trials
...
ttakenmean = struct;
convdpercent = struct;
for nL = 2:numLayersMax
    for nN = 1:numNeuronsMax
        errBCmean.("NL_" + nL + "_NN_" + nN) = 0;
        errICmean.("NL_" + nL + "_NN_" + nN) = 0;
        itermean.("NL_" + nL + "_NN_" + nN) = 0;
        fevalmean.("NL_" + nL + "_NN_" + nN) = 0;
        fstoptmean.("NL_" + nL + "_NN_" + nN) = 0;
        ttakenmean.("NL_" + nL + "_NN_" + nN) = 0;
        convdpercent.("NL_" + nL + "_NN_" + nN) = 0;
        for i = 1:30
            errBCmean.("NL_" + nL + "_NN_" + nN) = errBCmean.("NL_" + nL + "_NN_" + nN) + errBC.("NL_" + nL + "_NN_" + nN + "_TR_" + i);
            errICmean.("NL_" + nL + "_NN_" + nN) = errICmean.("NL_" + nL + "_NN_" + nN) + errIC.("NL_" + nL + "_NN_" + nN + "_TR_" + i);
            itermean.("NL_" + nL + "_NN_" + nN) = itermean.("NL_" + nL + "_NN_" + nN) + iter.("NL_" + nL + "_NN_" + nN + "_TR_" + i);
            fevalmean.("NL_" + nL + "_NN_" + nN) = fevalmean.("NL_" + nL + "_NN_" + nN) + feval.("NL_" + nL + "_NN_" + nN + "_TR_" + i);
            fstoptmean.("NL_" + nL + "_NN_" + nN) = fstoptmean.("NL_" + nL + "_NN_" + nN) + fstopt.("NL_" + nL + "_NN_" + nN + "_TR_" + i);
            ttakenmean.("NL_" + nL + "_NN_" + nN) = ttakenmean.("NL_" + nL + "_NN_" + nN) + ttaken.("NL_" + nL + "_NN_" + nN + "_TR_" + i);
            convdpercent.("NL_" + nL + "_NN_" + nN) = convdpercent.("NL_" + nL + "_NN_" + nN) + convd.("NL_" + nL + "_NN_" + nN + "_TR_" + i);
        end
        errBCmean.("NL_" + nL + "_NN_" + nN) = errBCmean.("NL_" + nL + "_NN_" + nN)/30;
        errICmean.("NL_" + nL + "_NN_" + nN) = errICmean.("NL_" + nL + "_NN_" + nN)/30;
        itermean.("NL_" + nL + "_NN_" + nN) = itermean.("NL_" + nL + "_NN_" + nN)/30;
        fevalmean.("NL_" + nL + "_NN_" + nN) = fevalmean.("NL_" + nL + "_NN_" + nN)/30;
        fstoptmean.("NL_" + nL + "_NN_" + nN) = fstoptmean.("NL_" + nL + "_NN_" + nN)/30;
        ttakenmean.("NL_" + nL + "_NN_" + nN) = ttakenmean.("NL_" + nL + "_NN_" + nN)/30;
        convdpercent.("NL_" + nL + "_NN_" + nN) = convdpercent.("NL_" + nL + "_NN_" + nN)*10/3;
    end
end

%% Plotting each of the metrics with respect to the nN and nL
nL0 = 2:numLayersMax;
nN0 = 1:numNeuronsMax;
nL = [];
nN = [];
for i = 1:numNeuronsMax
    nL = [nL, nL0'];
    
end
for i = 2:numLayersMax
    nN = [nN; nN0];
end
errICp = nL*0; 
errBCp = nL*0;
...
ttkp = nN*0;
cvdp = nN*0;
for nLi = 2:numLayersMax
    for nNi = 1:numNeuronsMax
        errBCp(nLi-1,nNi) = errBCmean.("NL_" + nLi + "_NN_" + nNi);
        errICp(nLi-1,nNi) = errICmean.("NL_" + nLi + "_NN_" + nNi);
        iterp(nLi-1,nNi) = itermean.("NL_" + nLi + "_NN_" + nNi);
        fevalp(nLi-1,nNi) = fevalmean.("NL_" + nLi + "_NN_" + nNi);
        ttkp(nLi-1,nNi) = ttakenmean.("NL_" + nLi + "_NN_" + nNi);
        cvdp(nLi-1,nNi) = convdpercent.("NL_" + nLi + "_NN_" + nNi);
    end
end
...
subplot(3,2,1)
    surf(nL,nN,errBCp);
    xlabel('Number of Layers $L$','Interpreter','latex');
    ylabel('Number of Neurons $N$','Interpreter','latex');
    zlabel('Error in the Boundary Conditions (MSE)','Interpreter','latex');
    set(gca,'ZScale','log');
subplot(3,2,2)
    surf(nL,nN,errICp)
    xlabel('Number of Layers $L$','Interpreter','latex');
    ylabel('Number of Neurons $N$','Interpreter','latex');
    zlabel('Error in the Initial Conditions (MSE)','Interpreter','latex');
    set(gca,'ZScale','log');
subplot(3,2,3)
    surf(nL,nN,iterp);
    xlabel('Number of Layers $L$','Interpreter','latex');
    ylabel('Number of Neurons $N$','Interpreter','latex');
    zlabel('Mean Number of Iterations ($iter$) until Convergence','Interpreter','latex');
subplot(3,2,4)
    surf(nL,nN,fevalp);
    xlabel('Number of Layers $L$','Interpreter','latex');
    ylabel('Number of Neurons $N$','Interpreter','latex');
    zlabel('Mean Number of Function Evaluations ($feval$) until Convergence','Interpreter','latex');
subplot(3,2,5)
    surf(nL,nN,ttkp);
    xlabel('Number of Layers $L$','Interpreter','latex');
    ylabel('Number of Neurons $N$','Interpreter','latex');
    zlabel('Mean Time elapsed until Convergence','Interpreter','latex');
subplot(3,2,6)
    surf(nL,nN,cvdp);
    xlabel('Number of Layers $L$','Interpreter','latex');
    ylabel('Number of Neurons $N$','Interpreter','latex');
    zlabel('Percentage of successful trials','Interpreter','latex');

%% Mean Number of iterations until Convergence
figure
surf(nL,nN,iterp);
xlabel('Number of Layers $L$','Interpreter','latex');
ylabel('Number of Neurons $N$','Interpreter','latex');
...
xlabel('Number of Layers $L$','Interpreter','latex');
ylabel('Number of Neurons $N$','Interpreter','latex');
title('Error in the Boundary Conditions (MSE)','Interpreter','latex');
colorbar
xlim([2,15]); ylim([1,20]);
subplot(1,2,2)
surf(nL,nN,errICp);
xlabel('Number of Layers $L$','Interpreter','latex');
ylabel('Number of Neurons $N$','Interpreter','latex');
title('Error in the Initial Conditions (MSE)','Interpreter','latex');
colorbar
xlim([2,15]); ylim([1,20]);
