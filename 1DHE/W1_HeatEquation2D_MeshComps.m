...
% This problem will solve the 1DHE in order to compare four different types
...
% f. Select the options for the fmincon algorithm. Could add the
...
outputs_all = struct;
flags_A = []; flags_B = []; flags_C = []; flags_D = [];
...
% Inputting the data on the inputs.
    ...
    inputs.dataT = tpts';
% Implementing the loop
for randomICPs = 0:99 % Originally 1:16
    % Implementing the solution of the model
    % Generate the number of parameters
    [parameters,seed] = generateParameters(numLayers,numNeurons,seed);
    % Implement the training of the model for the parameters
...
% This will be an relative Linfty error.
...
scatter(xx(yy>2e-2),yy(yy>2e-2),'rx');
hold off
set(gca,'YScale','log'); set(gca,'XScale','log'); box on
ylabel('Error in the Initial Conditions', 'Interpreter','latex');
xlabel('Number of iterations','Interpreter','latex');
eff = sum(yy<2e-2);
str = "Effectiveness = " + string(eff) + "/100";
...
hold off
set(gca,'YScale','log'); set(gca,'XScale','log'); box on
ylabel('Error in the Final Solution', 'Interpreter','latex');
xlabel('Number of iterations','Interpreter','latex');
eff = sum(yy<2e-2);
str = "Effectiveness = " + string(eff) + "/100";
title("Random Meshing with " + str,'Interpreter','latex')
...
% figure
% clf
% bubblechart(nevals(errICs < 0.01)', niters(errICs < 0.01)', telaps(errICs < 0.01)');
